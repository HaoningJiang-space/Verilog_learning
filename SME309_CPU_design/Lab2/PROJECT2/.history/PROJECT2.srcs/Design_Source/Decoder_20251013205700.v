    ...
    always@(*)begin
        case (Op)
            2'b00: begin // Data Processing
                ...
                MemW_r     = 1'b0;
                ALUSrc_r   = I;       // I=1: 立即数
                ImmSrc_r   = 2'b00;   // DP 立即数扩展
                RegW_r     = 1'b1;    // 缺省写回，具体在 ALU 解码里按指令修正
                RegSrc_r   = 2'b00;   // Rn / Rm
                ALUOp_r    = 2'b11;   // 标记为数据处理
            end
            ...
            2'b01: begin // LDR/STR
                ...
                RegW_r     = L;       // 仅 LDR 写回
                // 修正：RegSrc[0] 选择 A2=Rd(用于 STR)，RegSrc[1] 选择 A1=R15
                RegSrc_r   = {1'b0, ~L}; // STR 时 A2=Rd
                ALUOp_r    = 2'b10;   // 非数据处理（地址类）
            end
...
//-----------------
    always@(*)begin
        ...
        else if (ALUOp[0]==1'b1 && ALUOp[1]==1'b1) begin // 数据处理
            case (funct)
                4'b0100: begin // ADD
                    ALUControl_r = 2'b00;
                    FlagW_r      = Instr[20] ? 2'b11 : 2'b00; // 算术：S=1 更 NZCV
                    NoWrite_r    = 1'b0;
                end
                4'b0010: begin // SUB
                    ALUControl_r = 2'b01;
                ...
                4'b0000: begin // AND
                    ALUControl_r = 2'b10;
                    FlagW_r      = Instr[20] ? 2'b10 : 2'b00; // 逻辑：S=1 更 NZ
                    NoWrite_r    = 1'b0;
                end
                4'b1100: begin // ORR
                    ALUControl_r = 2'b11;
