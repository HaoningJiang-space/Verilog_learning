...
module Decoder(
    ...
    output        ALUSrc,
    output [1:0]  ImmSrc,
    output [1:0]  RegSrc,      // RegSrc[0]: A1 选 R15；RegSrc[1]: A2 选 Rd(STR)  ← 修正注释位义
    ...
    always @(*) begin
        ...
        case (op)
            2'b10: begin // Branch
                ALUSrc_d = 1'b1;
                ImmSrc_d = 2'b10;
                RegSrc_d = 2'b01;     // 修正：A1=R15（RegSrc[0]=1）
                PCS_d    = 1'b1;
            end
            2'b01: begin // LDR/STR
                MemtoReg_d = L;
                ...
                ALUSrc_d   = 1'b1;
                ImmSrc_d   = 2'b01;
                RegSrc_d   = {~L, 1'b0}; // 修正：STR 时 A2=Rd（RegSrc[1]=1）
            ...
            2'b00: begin // Data Processing
                ...
                case (funct)
                    ...
                    default: begin
                        ...
                        // S=1：逻辑类(AND/ORR)更 NZ；算术类(ADD/SUB)更 NZCV
                    end
                endcase
                if (RegW_d && (Rd==4'd15)) PCS_d = 1'b1;
            end
