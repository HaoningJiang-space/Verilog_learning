...
module ALU(
    ...
    // ALUControl encoding (typical for simple ARM lite):
        ...
        always @(*) begin
            ...
            endcase
            C = sum_ext[32] & ALUControl[1]; // In ARM, for subtraction, carry out represents NOT borrow when using A + (~B) + 1
            V = (~(Src_A[31] ^ Src_B[31] ^ ALUControl[0])) & (result_r[31] ^ Src_A[31]) & (~ALUControl[1]);
            N = result_r[31];
            Z = (result_r == 32'b0);
        end
