...
module Decoder(
    ...
    assign PCS      = (((Rd == 4'd15) & RegW_d)|Branch) ? PCS_d : 1'b0; // Only allow PC update if Rd is R15
    assign RegW     = RegW_d;
    assign MemW     = MemW_d;
        ...
        casex(op)
          ...
          2'b01: begin
            ...
            ImmSrc_d   = 2'b01;   // load/store immediate type
                ...
                RegSrc_d = {~L, 1'b0};
            if (RegW_d && (Rd==4'd15)) PCS_d = 1'b1; // 写R15置PCS
          end
          2'b00: begin
            case (funct[3:0])
