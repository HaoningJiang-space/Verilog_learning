...
module Decoder(
    ...
    // ALU 操作选择（DP/比较类映射，按 ALU: 00 ADD, 01 SUB, 10 AND, 11 ORR）
    always @(*) begin
        ALUControl = 2'b00; // 默认 ADD
        case (op)
            2'b10: ALUControl = 2'b00; // Branch: PC + offset -> ADD
            2'b01: ALUControl = 2'b00; // LDR/STR: base + offset -> ADD
            2'b00: begin // Data Processing
                case (funct[3:0])
                    ...
                    4'b1001: ALUControl = 2'b10; // TEQ -> AND（简化）
                    4'b1010: ALUControl = 2'b01; // CMP -> SUB
                    4'b1011: ALUControl = 2'b00; // CMN -> ADD
                    default: ALUControl = 2'b00; // 缺省 ADD
                endcase
            end
        endcase
    end
